...
class RawIODFasterRCNN(IODModel):
    """
    Same as IODFasterRCNN, but the box head does not adapt to the dataset. Should only work on MSCOCO.
    """

    def __init__(self, exp: IODExperiment, num_classes, freeze_backbone=True, freeze_rpn=False, **kwargs):
        super().__init__(exp)
        self._exp = exp
        #
        # model_kwargs = dict(
        ...
        # self._model: FasterRCNN = fasterrcnn_resnet50_fpn(pretrained=True, **model_kwargs).to(self._device)
        #
        #
        ...
        # self._checkpoint_name = "{0}.pth".format(RawIODFasterRCNN.__name__)
    ...
    def forward(self, patterns, targets=None) -> Tuple[Any, Any]:
        map = self._exp.current_dataset._loader._id_map
        res = self._model(patterns, targets)

        for idx, r in enumerate(res):
            try:
                res[idx]["labels"] = r["labels"].cpu().apply_(lambda x: map[x]).to(self._device)
    ...
    def restore(self, exp: "IODExperiment", checkpoint_path: Path):
        ...
        try:
            ...
            self._logger.log_message(
                "No checkpoint found for model {0} at {1}".format(RawIODFasterRCNN.__name__, model_path),
    ...
    def construct_optimizer(self):
        ...
        return torch.optim.SGD(
            params, lr=0.001, momentum=0.9, nesterov=True, weight_decay=0.0005
        )
