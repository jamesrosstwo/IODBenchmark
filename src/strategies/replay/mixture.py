...
class GaussianMixtureStrategy(ReplayStrategy):
    """
    Overall idea:
    - fit a mixture of gaussians for each class
        - Each MoG will have one component for each datapoint we want to store for that class
        - This should choose a decent "core set" of exemplars for each class
        - We also want to determine which kind of datapoints would cause the most confusion, and store those in the
          memory. For this, we can sum the likelihoods generated by every MoG for each datapoint. The datapoints with
          the highest summed likelihood should be the most confusing exemplars, as their features probably lie between
          a variety of classes.
        - From this point on, with this set of fit MoGs, we can estimate distribution shift. This can be done by
          comparing the average likelihoods of each datapoint under each MoG. E.g if class one has 20 datapoints, we
          can calculate the average likelihood of those datapoints under the MoG fit to class two's data. Higher average
          likelihood would mean more confusion, and should correspond with more confusion.
    """
    ...
    def _update_buffer(self, split: IODSplit):
        ...
        for class_idx in split.labels:
            ...
            self._buffer.push([split[i].detach().cpu() for i in exemplar_indices])
