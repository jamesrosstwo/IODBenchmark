...
def copy_params_dict(model, copy_grad=False, should_cpu=False):
    """
    ...
    adj = lambda p: p.data.clone()
    if copy_grad:
        adj = lambda p: p.grad.data.clone()

    if should_cpu:
        adj = lambda p: adj(p).cpu()

    if should_cpu:
        adj = lambda p: adj(p).cpu()

    return [(k, adj(p)) for k, p in model.named_parameters()]
